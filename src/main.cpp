...
const string strMessageMagic = "MalaysiaCoin Signed Message:\n";
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 4500 MYCOIN if made of 50 MYCOIN inputs)
            if (nBytes < 10000)
                nMinFee = 0;
        }
        else
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MalaysiaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MalaysiaCoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("MalaysiaCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("malaysiacoin-miner");
    ...
    try { loop {
        ...
        printf("Running MalaysiaCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("MalaysiaCoinMiner terminated\n");
        throw;
    }
}
