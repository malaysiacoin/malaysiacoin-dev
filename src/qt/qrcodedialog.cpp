...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("MYCOIN")
    updateDisplayUnit();
}
...
{
    QString ret = QString("MalaysiaCoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non MYCOIN unit input in lnReqAmount, we generate the URI with MYCOIN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::MYCOIN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
